...
{
    [RoutePrefix("api/CustomerStatus")]
    [ClaimsAuthorize]
    public class CustomerStatusController : BaseController
    {
        ...
        [HttpPost]
        [Route("")]
        public async Task<HttpResponseMessage> Insert(CustomerStatusCreate model)
        {
        ...
        [HttpPut]
        [Route("")]
        public async Task<HttpResponseMessage> Update(CustomerStatusUpdate model)
        {
            if (ModelState.IsValid)
            {
                try
