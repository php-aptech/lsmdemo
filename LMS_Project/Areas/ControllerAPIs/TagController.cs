...
namespace LMS_Project.Areas.ControllerAPIs
{
    ...
    public class TagController : BaseController
    {
        ...
        {
            var data = await TagService.GetById(id);
            if (data == null)
        ...
        [HttpGet]
        [Route("")]
        public async Task<HttpResponseMessage> GetAll([FromUri] TagSearch baseSearch)
        {
            var data = await TagService.GetAll(baseSearch);
            if (data.TotalRow == 0)
                return Request.CreateResponse(HttpStatusCode.NoContent);
