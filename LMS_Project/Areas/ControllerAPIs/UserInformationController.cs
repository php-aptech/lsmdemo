    ...
    public class UserInformationController : BaseController
    {
        ...
        public async Task<HttpResponseMessage> Insert(UserCreate model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var data = await UserInformation.Insert(new tbl_UserInformation(model), GetCurrentUser(),model.ProgramIds);
                    if (data != null && model.CustomerId != 0 && model.CustomerId.HasValue)
                    {
                        //cập nhật trạng thái khách hàng 
                        await CustomerService.Complete(model.CustomerId.Value);
                    }
        ...
        public async Task<HttpResponseMessage> Update_OneSignal_DeviceId(string oneSignal_deviceId)
        {
                        ...
                        if (Inputfile != null && FileStream != null)
                        {
                            ...
                            if (dsexcelRecords != null && dsexcelRecords.Tables.Count > 0)
                            {
                                ...
                                for (int i = 3; i < dtStudentRecords.Rows.Count; i++)
                                {
                                    ...
                                    {
                                        return Request.CreateResponse(HttpStatusCode.BadRequest, new { message = "Vui lòng điền đầy đủ họ tên và tên đăng nhập" });
                                    }
                                    model.Add(item);
                                }
                            }
        ...
        public async Task<HttpResponseMessage> GetUserAvailable(int roleId)
        {
            var data = await UserInformation.GetUserAvailable(roleId, GetCurrentUser());
            if (!data.Any())
                return Request.CreateResponse(HttpStatusCode.NoContent);
