...
namespace LMS_Project.Areas.ControllerAPIs
{
    ...
    public class PurposeController : BaseController
    {
        ...
        {
            var data = await PurposeService.GetById(id);
            if (data == null)
        ...
        [HttpPost]
        [Route("")]
        public async Task<HttpResponseMessage> Insert(PurposeCreate model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var data = await PurposeService.Insert(model, GetCurrentUser());
        ...
        [HttpPut]
        [Route("")]
        public async Task<HttpResponseMessage> Update(PurposeUpdate model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var data = await PurposeService.Update(model, GetCurrentUser());
                    return Request.CreateResponse(HttpStatusCode.OK, new { message = "Thành công !", data });
                }
                catch (Exception e)
                {
        ...
        {
            var data = await PurposeService.GetAll(baseSearch);
            if (data.TotalRow == 0)
                return Request.CreateResponse(HttpStatusCode.NoContent);
