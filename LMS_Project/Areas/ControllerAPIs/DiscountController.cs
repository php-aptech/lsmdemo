...
namespace LMS_Project.Areas.ControllerAPIs
{
    ...
    public class DiscountController : BaseController
    {
        ...
        [HttpPost]
        [Route("")]
        public async Task<HttpResponseMessage> Insert(DiscountCreate model)
        {
        ...
        [HttpPut]
        [Route("")]
        public async Task<HttpResponseMessage> Update(DiscountUpdate model)
        {
            if (ModelState.IsValid)
            {
                try
        ...
        [HttpGet]
        [Route("")]
        public async Task<HttpResponseMessage> GetAll([FromUri] DiscountSearch baseSearch)
        {
            var data = await DiscountService.GetAll(baseSearch);
            if (data.TotalRow == 0)
                return Request.CreateResponse(HttpStatusCode.NoContent);
