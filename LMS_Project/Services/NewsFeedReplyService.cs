        ...
        public static async Task<AppDomainResult> GetAll(NewsFeedReplySearch baseSearch)
        {
            using (var db = new lmsDbContext())
            {
                ...
                var parms = new List<SqlParameter>
                {
                    ...
                    new SqlParameter { ParameterName = "@NewsFeedCommentId", Value= baseSearch.NewsFeedCommentId == null ? 0 : baseSearch.NewsFeedCommentId,DbType = DbType.Int64 }
                };
                var sqlStr = $"Exec Get_NewsFeedReply @PageIndex,@PageSize,@NewsFeedCommentId";
                var data = await db.Database.SqlQuery<Get_NewsFeedReply>(sqlStr, parms.ToArray()).ToListAsync();
                if (!data.Any())
