    ...
    public class FileCurriculumInClassSerivce
    {
        ...
        public static async Task<List<Get_FileCurriculumInClass>> GetFileCurriculumInClass(FilesCurriculumInClassSearch baseSearch,tbl_UserInformation userLog)
        {
            using (var db = new lmsDbContext())
            {
                ...
                {
                    new SqlParameter { ParameterName = "@CurriculumDetailId", Value= baseSearch.CurriculumDetailInClassId == null ? 0 : baseSearch.CurriculumDetailInClassId, DbType = DbType.Int64},
                    new SqlParameter { ParameterName = "@RoleId", Value= userLog.RoleId, DbType = DbType.Int64}
                };
                var sqlStr = $"Exec Get_FileCurriculumInClass @CurriculumDetailId,@RoleId";
                var data = await db.Database.SqlQuery<Get_FileCurriculumInClass>(sqlStr, parms.ToArray()).ToListAsync();
                return data;
            }
        }
