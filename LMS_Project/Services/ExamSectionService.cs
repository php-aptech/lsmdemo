...
namespace LMS_Project.Services
{
    ...
    public class ExamSectionService
    {
            ...
            using (var db = new lmsDbContext())
            {
                using (var tran = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        tran.Commit();
                        await ReloadIndex(entity.ExamId.Value);
                        await ExerciseGroupService.UpdateExerciseNumber(entity.ExamId.Value);
                    }
                    catch (Exception e)
                    {
                        tran.Rollback();
        ...
        /// <returns></returns>
        public static async Task ReloadIndex(int examId)
        {
            using (var db = new lmsDbContext())
            {
                var examSections = await db.tbl_ExamSection.Where(x => x.ExamId == examId && x.Enable == true).OrderBy(x=>x.Index).ToListAsync();
                if (examSections.Any())
                {
                    int index = 1;
                    foreach (var item in examSections)
                    {
                        var examSection = await db.tbl_ExamSection.SingleOrDefaultAsync(x => x.Id == item.Id);
