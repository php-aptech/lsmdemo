    ...
    public class NewsFeedService
    {
        public static async Task<tbl_NewsFeed> Insert(NewsFeedCreate itemModel, tbl_UserInformation userLogin)
        {
            using (var db = new lmsDbContext())
            {
                ...
                if (itemModel.FileListCreate != null)
                {
        ...
        public static async Task<AppDomainResult> GetAll(NewsFeedSearch baseSearch, tbl_UserInformation userLogin)
        {
            using (var db = new lmsDbContext())
            {
                ...
                var parms = new List<SqlParameter>
                {
                    ...
                    new SqlParameter { ParameterName = "@BranchIds", Value= baseSearch.BranchIds == null ? 0 : baseSearch.BranchIds,DbType = DbType.Int64 },
                    new SqlParameter { ParameterName = "@MyBranchIds", Value= userLogin.RoleId == ((int)RoleEnum.admin) ? "" : userLogin.BranchIds,DbType = DbType.String },
                    ...
                    new SqlParameter { ParameterName = "@RoleId", Value= userLogin.RoleId,DbType = DbType.Int64 },
                };
                var sqlStr = $"Exec Get_NewsFeed @PageIndex,@PageSize,@NewsFeedGroupId,@BranchIds,@MyBranchIds,@AccountLogin,@RoleId";
                var data = await db.Database.SqlQuery<Get_NewsFeed>(sqlStr, parms.ToArray()).ToListAsync();
                if (!data.Any())
                    return new AppDomainResult { Data = null, TotalRow = 0, Success = true };
                int totalRow = data[0].TotalRow ?? 0;
                var result = data.Select(i => new tbl_NewsFeed(i)).ToList();
        ...
        public static async Task<tbl_NewsFeed> GetById(int id, tbl_UserInformation userLogin)
        {
            using (var db = new lmsDbContext())
            {
                ...
                if (entity.NewsFeedGroupId != null && entity.NewsFeedGroupId != 0)
                {
                    ...
                    entity.TypeNameGroup = userInGroup.TypeName ?? string.Empty;
                }                
                //Info creater
