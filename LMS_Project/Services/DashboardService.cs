    ...
    public class DashboardService
    {
        ...
        public static async Task<List<StatisticialCustomerInMonth>> StatisticialCustomerInMonth(StatisticialCustomerSearch baseSearch)
        {
            using (var db = new lmsDbContext())
            {
                ...
                string sql = "GetStatistical_Chart_CustomerInMonth @BranchIds = {0}";
                var data = await db.Database.SqlQuery<StatisticialCustomerInMonth>(sql, baseSearch.BranchIds).ToListAsync();
        ...
        public static async Task<List<StatisticialAllCustomer>> StatisticialAllCustomer(StatisticialCustomerSearch baseSearch)
        {
            ...
            {
                if (baseSearch == null) baseSearch = new StatisticialCustomerSearch();
                string sql = "GetStatistical_Chart_AllCustomer @BranchIds = {0}";
                var data = await db.Database.SqlQuery<StatisticialAllCustomer>(sql, baseSearch.BranchIds).ToListAsync();
                return data;
            }
