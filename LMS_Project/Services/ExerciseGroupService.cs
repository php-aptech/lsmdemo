...
namespace LMS_Project.Services
{
    public class ExerciseGroupService
    {
            ...
            using (var db = new lmsDbContext())
            {
                using (var tran = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        tran.Commit();
                        await UpdateExerciseNumber(examSection.ExamId.Value);
                    }
                    catch (Exception e)
                    {
                        tran.Rollback();
        ...
        /// <returns></returns>
        public static async Task UpdateExerciseNumber(int examId)
        {
            using (var db = new lmsDbContext())
            {
            ...
            using (var db = new lmsDbContext())
            {
                using (var tran = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        tran.Commit();
                        await UpdateExerciseNumber(exerciseGroup.ExamId.Value);
                    }
                    catch (Exception e)
                    {
                        tran.Rollback();
            ...
            using (var db = new lmsDbContext())
            {
                using (var tran = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        await db.SaveChangesAsync();
                        tran.Commit();
                        await ReloadIndex(entity.ExamSectionId.Value);
                        await UpdateExerciseNumber(entity.ExamId.Value);
                    }
                    catch (Exception e)
                    {
                        tran.Rollback();
        ...
        /// <param name="examId"></param>
        /// <returns></returns>
        public static async Task ReloadIndex(int examSectionId)
        {
            using (var db = new lmsDbContext())
            {
